...
namespace Lykke.Service.DevCerts.Code
{
    ...
    public class GoogleJsonWebSignatureEx
    {
        ...
        internal static async Task<Payload> ValidateInternalAsync(string jwt, IClock clock, bool forceGoogleCertRefresh, string certsJson)
        {
            ...
            // Rounding to upper integer and adds one extra second to allow google's clock and our clock be a little bit different
            var nowSeconds = (long)Math.Ceiling((clock.UtcNow - UnixEpoch).TotalSeconds) + 1;
